...
function WarMenu.LaotLeftRight(id, x, y, width)
	menus[id].x = x or 0.0175
	menus[id].y = y or 0.025
	menus[id].width = width or 0.23
end

function WarMenu.CreateMenu(id, title, subtitle, r, g, b)
	-- Default settings
	menus[id] = { }
	...
	menus[id].titleBackgroundColor = { r = r, g = g, b = b, a = 255 }
...
function WarMenu.SetButtonScale(scale)
	buttonScale = scale or 0.3
end
...
function WarMenu.CheckBox(text, checked, callback)
	if WarMenu.Button(text, checked and '☑️' or '🔲') then
		checked = not checked
